...
def generate_gnuplot_script(ens, dt):
  with open("%s_adaptive.gnuplot" %ens, 'w') as f:
    print("set terminal epslatex size 16.00cm, 8.00cm color lw 2 standalone colortext font 10 \\", file=f)
    ...
    print("", file=f)
    print("outfile = '%s_adaptive'    # Also used for filename generation so set it" %ens, file=f)
    print("", file=f)
    print("set key spacing 1.2", file=f)
    ...
    print("", file=f)
    print("plot for [i=1:7:1] infile using (2-2*cos($1*two_pi)+0.04*i):(column(2*i)):(column(2*i+1)) with ye lw 1.5 pt 7 title columnheader(i*2), \\", file=f)
    print("", file=f)
    print("set output # Need to close the output file before latex can work with it", file=f)
    print("", file=f)
    print("system 'pdflatex -shell-escape -interaction=batchmode '.outfile.\\", file=f)
    print("	'.tex > /dev/null; rm '.outfile.'-inc.eps '.outfile.\\", file=f)
    print("		'-inc-eps-converted-to.pdf '.outfile.'.tex $(basename '.\\", file=f)
    print("			outfile.').log $(basename '.outfile.').aux'\\", file=f)
    print("", file=f)
...
def compute(filename_stem, jack, T, tau_limit):
  ...
  return [ 0.5*np.log((fft_data[0+0][n].real/fft_data[0+1][n].real)**2) for n in range(T)]

def compute_ens(ens_name, Njack, T, tau_limit, dtau, fit_min, fit_max, label):
  ...
  for jack in range(Njack):
    jlst = compute(filename_stem, jack, T, tau_limit)
...
def compute_ens_wfs(ens_name, Njack, T, tau_limit, dtau, fit_min, fit_max, label):
  fft = []
  for dir_path in os.listdir('../Qslice_to_correlation'):
    if fnmatch.fnmatch(dir_path, "%s_wfs*"%(ens_name)):
      ...
      # print wfs
      fft.append([wfs, compute_ens(dir_path, Njack+1, T, tau_limit, dtau, fit_min, fit_max, label)])
  f = open(label, 'w')
  # print fft
  f.write("wfs     ")
  for w in range(len(fft)):
      # f.write( "%6.4f %12.8 %12.8\n" %(np.fft.fftfreq(T)[n], fft[w][1][n][0], fft[w][1][n][1]) )  
      f.write( "wfs%+.3f     wfs%+.3f     " % (fft[w][0], fft[w][0]) )  
  f.write("\n")
  for n in range(T):
...
compute_ens_wfs(ens_stem, njack, ens_t, 5, ens_dtau, 0, 6, "%s_adaptive.dat"%ens_stem)

generate_gnuplot_script(ens_stem, ens_dtau)
os.system("gnuplot -c "+ ens_stem +"_adaptive.gnuplot")
